...
export class AppComponent {
  ...
  books = [
    {
      ...
      "author": "Maeve Binchy",
      "summary": "ttiends met Minnie rollen.",
    ...
    },
    {
      ...
      "author": "Dan Brown",
      "summary": "Whompaniedhe world into controversy..",
    ...
    },
    {
      ...
      "title": "Last Minute",
      "author": "Linda van Rijn",
      "summary": "f moeten afrekenen met haar verleden.",
    ...
    },
    {
      ...
      "author": "Khaled Hosseini",
      "summary": " een verpletterend verhaal",
  ...
  insertBook(e: Book) {
    const temp = this.cartItems.findIndex((item) => item.id === e.id) ;
    if (temp !== -1 ){
      this.cartItems[temp].quantity = this.cartItems[temp].quantity! +1 ;
    }
    else {
      e.quantity = 1 ;
  ...
  increaseQuantity(id:number){
    let index = this.cartItems.findIndex((book)=>{return book.id == id});
    this.cartItems[index].quantity = this.cartItems[index].quantity! + 1;
    this.total = this.cartItems[index].price.value*this.cartItems[index].quantity!;
  }

  decreaseQuantity(id:number){
    let index = this.cartItems.findIndex((book)=>{return book.id == id});
    if(this.cartItems[index].quantity === 1){
      this.total -= this.cartItems[index].price.value! ;
  ...
  removeItem(id: number) {
    const itemIndex = this.cartItems.findIndex((obj) => obj.id === id);
    let total1:number = 0;
    if (itemIndex !== -1) {
      total1 = this.cartItems[itemIndex].price.value*this.cartItems[itemIndex].quantity!;
      this.total -= total1;
